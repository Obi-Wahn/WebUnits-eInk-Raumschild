...
def schleife_im_thread(conf, monkeypatch):
    ...
    monkeypatch.setattr(steuerung, "get_current_lesson", lambda c: ({"current": None, "next": None}, "Testlauf"))
...
def test_schleife_ueberlebt_eine_ausnahme(conf, monkeypatch, caplog, schleife_im_thread):
    ...
    monkeypatch.setattr(steuerung, "get_cached_config", zunaechst_kaputt)
...
def test_ausnahme_wird_mit_aufrufpfad_protokolliert(conf, monkeypatch, caplog, schleife_im_thread):
    ...
    monkeypatch.setattr(steuerung, "get_cached_config", immer_kaputt)
...
def test_schleife_beendet_sich_auf_signal(conf, monkeypatch):
    ...
    """
    monkeypatch.setattr(steuerung, "get_current_lesson", lambda c: ({"current": None, "next": None}, ""))
